...
var pdl = 130+130;
    ...
    function show_search()
    {
        ...
        else
        {
            ...
            $('#search-input').focus();
            $('#search-ovl').addClass('show').click(function(){hide_search()});
...
{
    popup('download','/download-verify/' + id,false,'','','')
}
    ...
    if(a.hasClass('loading')===false)
    {
        ...
        $.ajax({
            ...
            success: function(html)
            {
                if(html == '0')
                {
// nếu lỗi quay trở lại trang captcha theo mã lỗi
...
function load_ad(name,content,time)
{
    ...
    _now = _time();
    if(!p_a_t) {$.cookie(ad_cookie, _now, {expires: 30, path: '/'});
        p_a_t = 0;
    }
...
function popup(a,url,cache,method,data,cl)
{
    ...
    jQuery(function($)
    {
		...
		$('#search-button').click(function(){show_search()});
        setTimeout(function(){hide_bn_l_b('left')}, 5*60*1000);
        setTimeout(function(){hide_bn_l_b('right')}, 5*60*1000);
...
var menu_pos = 0;
	...
	//--- direction
        ...
        $("#left-menu .close").click(function(){hide_tinymenu()});
        $("#Fancy-Free").css('height',$(window).height());

        $("#search-input").keyup(function(){var s_v=$(this).val();if(s_v!="")$('#search .input .buttons').show();else $('#search .input .buttons').hide()});
...
function alignLeftRightBanners()
{
    ...
    $('#floatleft').css({'left': $(mainElement).position().left - leftBarWidth - 15, 'z-index': 4});
    $('#floatright').css({'left': $(mainElement).position().left + mainWidth + 20, 'z-index': 4});
}
function showSearch() {
    hide_main_menu()
    $('body').addClass('show-search');
